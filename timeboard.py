...
class timeboard:
    def __init__(self, game, title, leaderboard, player_pos, x = 0, y = 0, w = 300, h = 200, timer = None, client_manager = None):
        self.game = game
    ...
    def create_leaderboard_text(self):
        ...
        num_font_renderer = pg.font.SysFont(None, 24)
        ...
        for place in self.four_to_display:
            ...
            score = num_font_renderer.render(self.seconds_to_min(place[2]), True, colour_gradient[i])
    ...
    def live_draw(self):
        ...
        if self.timer == None or self.client_manager == None:
            sys.exit()
        else:
            #update the time leaderboard using current time
            self.client_manager.add_new_time(self.timer.get_time(), 0)

            #update our stored time
            self.update(self.client_manager.get_timeboard(0), self.client_manager.get_time_board_pos(0) + 1)
